...
def main() -> None:
    ...
    for i in range(1, 4):
        image = ArticleImage(image_path=generate_image_path(article1.id, i, "jpg"), sequence_number=i)
    ...
    for i in range(1, 3):
        video = ArticleVideo(video_path=generate_video_path(article1.id, i, "mp4"), sequence_number=i)
    ...
    for i in range(1, 6):
        image = ArticleImage(image_path=generate_image_path(article2.id, i, "png"), sequence_number=i)
    ...
    # Query by URL
    print("\n   Query 3: Find by URL")
    found_article = session.query(Article).filter(Article.url == "https://example.com/article/python-tutorial").first()
    if found_article:
    ...
    images = (
        ...
        .filter(ArticleImage.article_id == article1.id)
    ...
    image_count_before = session.query(ArticleImage).filter(ArticleImage.article_id == article_to_delete_id).count()
    print(f"   Images before delete: {image_count_before}")

    session.delete(article2)
    session.commit()

    image_count_after = session.query(ArticleImage).filter(ArticleImage.article_id == article_to_delete_id).count()
